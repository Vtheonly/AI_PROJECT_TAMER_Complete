...
v2.4 Changes (on top of v2.3):
  - Added offline pre-trained weight loading from a local Kaggle dataset.
    Set config.local_backbone_path to the path of model.safetensors.
  - pretrained=False when local_backbone_path is set (no internet needed).
  - pretrained=True fallback kept for online environments.
  - safetensors loading with head key removal for features_only=True compat.
...
v2.3 Changes:
  ...
  - Added row boundary markers: a learned "newline" embedding is inserted
    between each row of encoder patches.
"""
...
class SwinEncoder(nn.Module):
    def __init__(self, config):
        ...
        # ----------------------------------------------------------------
        # Decide whether to load pretrained weights from the internet
        # or from a local file (offline Kaggle environment).
        # ----------------------------------------------------------------
        ...
        # ----------------------------------------------------------------
        # Load weights from local safetensors file (offline mode)
        # ----------------------------------------------------------------
        if use_local_weights:
            ...
            logger.info("✅ Pre-trained weights loaded successfully (offline)")
        else:
            if want_pretrained:
                logger.info("✅ Pre-trained ImageNet weights loaded via timm (online)")
            else:
                logger.warning(
                    f"⚠️ local_backbone_path set but file not found: {local_weights_path} — training encoder from SCRATCH"
                )
